            ...
            <form action="{{ route('admin.users.update', $user->user_id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('role') is-invalid @enderror" id="role" name="role" required>
                        ...
                        <option value="college" {{ (old('role', $user->role) == 'college') ? 'selected' : '' }}>College User</option>
                        <option value="RUSA" {{ (old('role', $user->role) == 'RUSA') ? 'selected' : '' }}>RUSA User</option>
                    </select>
                    @error('role')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="mb-3 college-field" id="collegeFieldDiv">
                    ...
                    <select class="form-select @error('college_id') is-invalid @enderror" id="college_id" name="college_id">
                        ...
                        @foreach($colleges as $college)
                            <option value="{{ $college->college_id }}" {{ (old('college_id', $user->college_id) == $college->college_id) ? 'selected' : '' }}>
