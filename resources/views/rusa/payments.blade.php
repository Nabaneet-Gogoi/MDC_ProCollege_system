...
.reference-code {
    ...
    padding: 2px 6px;
    border-radius: 4px;
    border: 1px solid var(--rusa-tertiary);
}
...
@media (max-width: 576px) {
    ...
    <div class="rusa-header d-flex justify-content-between align-items-center">
        <h1><i class="bi bi-credit-card me-2"></i>Payments Monitoring</h1>
    ...
    <div class="compact-section">
        <div class="rusa-filter-card p-3">
            <div class="d-flex align-items-center mb-3">
                <h6 class="mb-0"><i class="bi bi-funnel me-2"></i>Filter Payments</h6>
            </div>
                ...
                <div class="col-md-12">
                    ...
                    <div class="search-input-group input-group">
                        <input type="text" class="form-control form-control-rusa" id="search" name="search" 
                            placeholder="Search payments by reference, college..." value="{{ request('search') }}">
                        <button type="submit" class="btn btn-rusa-primary">
                            <i class="bi bi-search"></i> Search
                        </button>
                        <a href="{{ route('rusa.payments') }}" class="btn btn-rusa-secondary">
    ...
    <div class="compact-section">
        <div class="rusa-card">
            <div class="rusa-card-header d-flex justify-content-between align-items-center">
                ...
                <span class="payments-count">{{ $payments->total() }}</span>
            </div>
            <div class="card-body p-0">
                @if(isset($payments) && count($payments) > 0)
                    <div class="table-responsive">
                        <table class="table table-rusa mb-0" id="paymentsTable">
                            <thead>
                                <tr>
                                    <th><i class="bi bi-receipt me-1"></i>Bill</th>
                                    ...
                                    <th><i class="bi bi-link-45deg me-1"></i>Reference</th>
                            ...
                            <tbody>
                                @foreach($payments as $payment)
                                    <tr>
                                        <td>
                                            <div class="fw-semibold">{{ $payment->bill->bill_no }}</div>
                                        </td>
                                        <td>{{ $payment->bill->college->college_name }}</td>
                                        <td>
                                            <span class="payment-amount">{{ number_format($payment->payment_amt, 2) }}</span>
                                        </td>
                                        ...
                                        <td>
                                            ...
                                            @elseif($payment->payment_status == 'processed')
                                                <span class="rusa-badge badge-processed">Processed</span>
                                            @elseif($payment->payment_status == 'completed')
                                                <span class="rusa-badge badge-completed">Completed</span>
                                        ...
                                        <td>
                                            @if($payment->transaction_reference)
                                                <code class="reference-code">{{ $payment->transaction_reference }}</code>
                                            @else
                                                <span class="text-muted">N/A</span>
                                            @endif
                                        </td>
                                        <td>
                                            <a href="{{ route('payments.print', $payment->payment_id) }}" class="btn btn-rusa-outline btn-sm" target="_blank">
                                                <i class="bi bi-printer"></i> Print
                                            </a>
                                        </td>
                                    </tr>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Animate payment amounts on load
        ...
        // Animate reference codes with typewriter effect
        const referenceCodes = document.querySelectorAll('.reference-code');
        referenceCodes.forEach((code, index) => {
            const originalText = code.textContent;
            code.textContent = '';
            setTimeout(() => {
                let i = 0;
                const typeInterval = setInterval(() => {
                    code.textContent += originalText[i];
                    i++;
